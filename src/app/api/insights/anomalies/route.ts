...
import { buildTenantInsights, computeAnomalies } from "@/lib/insights/tenant-insights";
...
export async function GET(request: Request) {
  ...
  const anomalies = computeAnomalies(spend);
  return NextResponse.json({ ok: true, tenant: ctx.tenant, range: parsed.data, anomalies });
}
