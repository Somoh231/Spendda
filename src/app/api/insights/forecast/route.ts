...
import { buildTenantInsights, computeForecast } from "@/lib/insights/tenant-insights";
...
export async function GET(request: Request) {
  ...
  const forecast = computeForecast(spend);
  return NextResponse.json({ ok: true, tenant: ctx.tenant, range: parsed.data, forecast });
}
