      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-6 sm:h-[4.25rem] lg:px-8">
        <a href="#top" className="group flex shrink-0 items-center gap-3">
          <span className="relative inline-flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white shadow-[0_12px_40px_rgba(37,99,235,0.22)] ring-1 ring-white/10 transition-transform duration-300 group-hover:scale-[1.02]">
            ...
            <img src={window.SPENDDA_LOGO} alt="Spendda" className="relative h-9 w-9 object-contain" />
          </span>
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
        >
          <window.Icon name={open ? "x" : "menu"} className="h-5 w-5" />
        </button>
      </div>
      ...
      {open && (
        <div className="border-t border-white/10 bg-slate-950/95 px-6 py-4 md:hidden">
          <nav className="flex flex-col gap-1">
            {LINKS.map((i) => (
              <a key={i.href} href={i.href} className="rounded-xl px-3 py-3 text-sm font-medium text-slate-200 hover:bg-white/5" onClick={() => setOpen(false)}>
                {i.label}
              </a>
            ))}
            <button onClick={() => { setOpen(false); onLogin(); }} className="rounded-xl px-3 py-3 text-left text-sm font-medium text-slate-200 hover:bg-white/5">Login</button>
            <a href="#cta" className="mt-1 rounded-xl bg-blue-500/15 px-3 py-3 text-center text-sm font-semibold text-blue-100" onClick={() => setOpen(false)}>Book demo</a>
          </nav>
        </div>
      )}
    </header>
